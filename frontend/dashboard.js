...
function updateDashboardState() {
    ...
    refreshWalletButton.disabled = !selectedWalletAddress;
    unlinkWalletButton.disabled = !selectedWalletAddress;
...
apiKeyList.addEventListener("click", async (event) => {
    ...
    if (!button || !currentSessionToken) return;
    ...
    try {
        ...
        if (button.dataset.revokeKey && currentApiKey && currentIssuedClientId === clientId) {
            currentApiKey = "";
        ...
        if (currentApiKey && currentIssuedClientId === clientId) {
            if (button.dataset.suspendKey) issuedKeyMeta.textContent = "The selected API key is temporarily disabled.";
            if (button.dataset.restoreKey) issuedKeyMeta.textContent = "The selected API key is active again.";
        }
        updateSnippets();
        updateDashboardState();
...
connectWalletButton.addEventListener("click", async () => {
    ...
    try {
        ...
        const challengeResponse = await fetch(`${API_BASE}/v1/accounts/me/solana/challenge`, {
            ...
            body: JSON.stringify({ wallet_address: walletAddress, provider: "phantom" }),
        });
        const challenge = await readApiResponse(challengeResponse);
        if (!challengeResponse.ok) throw new Error(JSON.stringify(challenge));
        ...
        if (!linkResponse.ok) throw new Error(JSON.stringify(linkedWallet));
...
createPolicyButton.addEventListener("click", async () => {
    ...
    try {
        const response = await fetch(`${API_BASE}/v1/policies`, {
            method: "POST",
            headers: { ...apiHeaders(), "Idempotency-Key": `portal-${Date.now()}` },
            body: JSON.stringify({
...
runAuthorizeButton.addEventListener("click", async () => {
    if (!currentApiKey || !currentPolicyId) return;
    try {
